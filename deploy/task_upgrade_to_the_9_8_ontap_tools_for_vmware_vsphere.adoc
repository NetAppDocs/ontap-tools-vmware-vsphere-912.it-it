...
. Estrarre la chiave pubblica dal certificato di firma del codice rilasciato da Entrust (OTV_ISO_CERT.pem)
...
Se il sistema storage viene aggiunto con un utente SVM scoped creato personalizzato utilizzando il file json e vuoi eseguire l'upgrade a 9,12
...
* Dopo aver eseguito l'aggiornamento alla versione più recente dei tool ONTAP con FIPS attivato, ma si dispone di una versione precedente di vCenter in cui FIPS non è supportato, l'implementazione funzionerà comunque.
